...
alphas, max_pages = np.meshgrid(alphas, max_pages)
...
surface = ax.plot_surface(alphas, max_pages, time, cmap=cm.coolwarm,
                          linewidth=0, antialiased=False)
ax.set_xlabel('Alpha')
ax.set_ylabel('Max overflow pages')
ax.set_zlabel('Time')
plt.savefig("data/plots/plot_time_delete_1000.png")

fig = plt.figure()
ax = fig.gca(projection='3d')
surface = ax.plot_surface(alphas, max_pages, disk_operations, cmap=cm.coolwarm,
                          linewidth=0, antialiased=False)
ax.view_init(30, 60)
ax.set_xlabel('Alpha')
ax.set_ylabel('Max overflow pages')
